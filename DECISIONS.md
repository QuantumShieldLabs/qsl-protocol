...
- **ID:** D-0322
  ...
  - **Date:** 2026-03-26
  - **Goals:** G4, G5
  - **Decision:** `NA-0206C` concludes that the final post-`w0` receive-retirement boundary still cannot be frozen from current merged evidence without semantic invention. Current `main` truthfully freezes only the live boundary: explicit `w0` rollback/coexistence remains available, already-supported `file_chunk` / `file_manifest` payloads remain supported while `w0` is live, and qsl-attachments/qsl-server add no new correctness blocker. But no merged artifact yet defines what receiver contract replaces that boundary once `w0` is retired or replaced. The truthful result is therefore `PR2` / `PB2`: do not advance to runtime retirement implementation, do not close out to continued-support posture, and do not invent a successor queue item without explicit new direction.
  - **Invariants:**
    - While `w0` remains live on current `main`, receive compatibility for already-supported legacy payloads remains required.
    - This decision does not authorize any immediate post-`w0` hard-reject rule, bounded drain-window receive rule, or intentional continued-support rule; none is yet frozen by merged evidence.
    - Current rollback/fallback, no-silent-fallback, honest-delivery, route-token/header, qsl-server transport-only, and qsl-attachments opaque ciphertext-only invariants remain unchanged.
    - `NA-0206C` remains unresolved for governance closeout until explicit post-`w0` receiver semantics are provided or approved.
  - **Alternatives Considered:**
    - Advance directly to receive-retirement implementation / `NA-0207` (rejected: would require selecting post-`w0` receiver semantics that current evidence does not freeze).
    - Close out to continued support / `NA-0206B` (rejected: current evidence still does not justify intentional longer-lived legacy receive posture as the truthful next product state).
    - Treat disappearance of new legacy sends as automatically sufficient to reject all later legacy receives (rejected: not frozen by current canonical or design docs and could silently break already-supported in-flight payloads).
