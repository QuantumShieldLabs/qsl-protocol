...
- **ID:** D-0376
  ...
  - **Date:** 2026-04-05
  - **Goals:** G4, G5
  - **Decision:** `NA-0221` implementation/evidence applies the `NA-0220` `P1` fail-closed remediation on refreshed `main` without reopening governance scope: qsc Suite-2 handshake establishment now rejects before any `hs_pending_store(...)` or `qsp_session_store(...)` when the primary authenticated peer pin is absent, the handshake initializer no longer hardcodes authenticated establishment when that precondition is missing, and the directly conflicting protected tests in `qsl/qsl-client/qsc/tests/identity_binding.rs` and `qsl/qsl-client/qsc/tests/identity_foundation_contract_na0217d.rs` are updated so first-contact TOFU establishment is retired on both responder and initiator paths while verified/pinned paths remain green. This decision is implementation/evidence only, not closeout, and queue promotion remains pending.
  - **Invariants:**
    - The remediation stays bounded to `qsl/qsl-client/qsc/src/handshake/**`, the authorized handshake/protected test files, `DECISIONS.md`, and `TRACEABILITY.md`; it does not alter transport, attachments, TUI, qsc-desktop, qsl-server, qsl-attachments, `.github`, cargo manifests, or website/public-runtime surfaces.
    - Transcript binding, pinned mismatch reject behavior, zero pending/session mutation on unknown or otherwise unauthenticated establishment attempts, existing tamper/no-mutation behavior, route-token/header discipline, honest-delivery semantics, qsl-server transport-only posture, and qsl-attachments opaque ciphertext-only posture remain protected.
    - The `P2` status-honesty follow-up remains out of scope except for wording-neutral marker truth required by the fail-closed `P1` fix.
  - **Alternatives Considered:**
    - Resume or force-push PR #657 instead of superseding from refreshed `main` (rejected: stale branch state is not authoritative after D-0374/D-0375 governance repair, and this directive requires a fresh implementation lane).
    - Preserve initiator-side or responder-side first-contact TOFU convenience in the protected tests (rejected: conflicts with merged canonical/audit truth and the repaired `NA-0221` policy/scope).
    - Broaden the remediation into transport, service, or unrelated runtime surfaces to chase secondary fallout (rejected: the approved seam is the bounded handshake fail-closed surface plus the two protected test files).
  - **References:** NA-0220; NA-0221; D-0375; PR #657; `NEXT_ACTIONS.md`; `TRACEABILITY.md`; `qsl/qsl-client/qsc/src/handshake/mod.rs`; `qsl/qsl-client/qsc/tests/handshake_security_closure.rs`; `qsl/qsl-client/qsc/tests/handshake_contract_na0217i.rs`; `qsl/qsl-client/qsc/tests/handshake_mvp.rs`; `qsl/qsl-client/qsc/tests/identity_binding.rs`; `qsl/qsl-client/qsc/tests/identity_foundation_contract_na0217d.rs`; `qsl/qsl-client/qsc/tests/qsp_protocol_gate.rs`; `docs/canonical/DOC-CAN-003_QSP_Suite-2_True_Triple_Ratchet_v5.0.0_DRAFT.md`; `docs/audit/DOC-AUD-002_qsc_Handshake_Execution_Security_Audit_v0.1.0_DRAFT.md`
