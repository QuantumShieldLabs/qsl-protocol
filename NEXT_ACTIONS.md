...
4) update conflicting first-contact TOFU expectations in both `identity_binding.rs` and `identity_foundation_contract_na0217d.rs` so protected tests match the fail-closed authenticated-establishment policy
...
2) protected initiator-side and responder-side first-contact TOFU expectations no longer contradict the canonical authenticated-establishment requirement
...
5) implementation validation passes, including `handshake_security_closure`, `qsp_protocol_gate`, `identity_binding`, `identity_foundation_contract_na0217d`, and the current cross-seam canaries
