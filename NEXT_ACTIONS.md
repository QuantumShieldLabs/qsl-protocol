...
Closeout evidence:
- closeout path: `CJ1`
- qsl-protocol implementation PR: #635 https://github.com/QuantumShieldLabs/qsl-protocol/pull/635
- qsl-protocol implementation merge SHA: `b227872f9f59`
- qsl-protocol implementation mergedAt: `2026-04-01T03:00:54Z`
- archive evidence: `docs/archive/testplans/NA-0217E_contacts_trust_routing_subsystem_extraction_evidence.md`
- exact implementation/evidence outcome:
  - merged main now carries `qsl/qsl-client/qsc/src/contacts/mod.rs`, the updated `qsl/qsl-client/qsc/tests/relay_auth_header.rs` and `qsl/qsl-client/qsc/tests/trust_remediation_ux_na0178.rs` regressions, `DECISIONS.md` `D-0353`, and the matching `TRACEABILITY.md` implementation/evidence entry, so the contacts / trust / routing extraction no longer relies only on ephemeral shell output for its no-drift truth.
  - the merged seam shrinks `qsl/qsl-client/qsc/src/main.rs` from `20,149` to `18,445` LOC while moving `1,713` LOC of contacts / trust / routing subsystem code into `qsl/qsl-client/qsc/src/contacts/mod.rs`.
  - CI exposed CodeQL alert `#95`, and the merged in-scope follow-up kept the human-facing verify-vs-trusted hint text unchanged while clearing the sink-level alert and preserving the trust-remediation CLI contract.
  - representative no-drift proof stayed explicit across primary-device routing, trust remediation / blocked-peer behavior, route-token normalization, the qsc-desktop-sensitive contacts/store suite, the honest-delivery canary, the `NA-0217D` identity canary, the `NA-0217C` protocol-state canary, the `NA-0217B` fs-store canary, and the `NA-0217A` marker/output canary.
  - this closeout PR is governance-only and introduces no runtime-path changes.
...
Scope:
- `qsl/qsl-client/qsc/src/main.rs`
- new `qsl/qsl-client/qsc/src/timeline/**`
- `qsl/qsl-client/qsc/tests/**` and qsl-protocol docs/evidence/governance only as needed for no-drift proof
- no qsc-desktop, qsl-server, or qsl-attachments runtime changes
- no `.github`, website, `Cargo.toml`, or `Cargo.lock` changes
Must protect:
- one `qsc` binary and the current CLI contract
- honest-delivery semantics, especially `accepted_by_relay != peer_confirmed`
- no mutation on reject
...
- qsl-attachments remains opaque ciphertext-only
Deliverables:
1) extract timeline store/load/save helpers, state-transition helpers, confirmation-apply helpers, and delivery-state emitters into a dedicated timeline subsystem module
2) keep existing call sites behavior-identical while reducing `main.rs` responsibility concentration
3) prove no drift across the representative timeline / delivery regression suites
4) update queue/evidence truthfully
Acceptance:
1) timeline persistence, delivery-state transitions, confirmation apply semantics, and honest-delivery behavior remain unchanged for the same inputs
...
Status: DONE
Implementation note:
- PR #671 is now merged on refreshed `main`; this closeout records durable archive evidence for the bounded TUI controller state / command-flow decomposition and promotes the next truthful successor without reopening runtime scope.
Problem:
...
Closeout evidence:
- closeout path: `CX1`
- qsl-protocol implementation PR: #671 https://github.com/QuantumShieldLabs/qsl-protocol/pull/671
- qsl-protocol implementation merge SHA: `1b1dbc22009d`
- qsl-protocol implementation mergedAt: `2026-04-07T03:18:57Z`
- archive evidence: `docs/archive/testplans/NA-0225_qsc_tui_controller_state_command_flow_decomposition_evidence.md`
- exact implementation/evidence outcome:
  - refreshed merged main now carries `DECISIONS.md` `D-0386`, the `TRACEABILITY.md` `NA-0225 implementation/evidence` entry, the merged `qsl/qsl-client/qsc/src/tui/controller.rs` orchestration shell, the child modules under `qsl/qsl-client/qsc/src/tui/controller/**`, and the merged `qsl/qsl-client/qsc/tests/tui_command_catalog_invariants.rs` surface from PR #671, so the bounded TUI controller decomposition is durable on `main` without relying on stale branch or PR state.
  - `qsl/qsl-client/qsc/src/tui/controller.rs` concentration was reduced from `9,417 / 9,839` TUI lines (`95.71%`) to `451 / 9,878` TUI lines (`4.57%`), while controller state, command-flow mediation, and render orchestration now live in smaller TUI-local modules under `qsl/qsl-client/qsc/src/tui/controller/**`.
  ...
  - the implementation landed on PR #671 from refreshed `main`, protected CI completed green before merge, and this closeout PR is governance-only with no runtime-path changes.
  - the next truthful successor is `NA-0226 — qsc TUI Command Mediation / Help-Catalog Decomposition` because refreshed residual TUI-local metrics show `qsl/qsl-client/qsc/src/tui/controller/commands.rs` is now the dominant remaining concentration inside `qsl/qsl-client/qsc/src/tui/controller/**` at `4,024 / 9,456` lines (`42.55%`), ahead of `state.rs` at `3,787 / 9,456` (`40.05%`), `render.rs` at `1,194 / 9,456` (`12.63%`), and the retained shell at `451 / 9,456` (`4.77%`).

---

### NA-0226 — qsc TUI Command Mediation / Help-Catalog Decomposition
Status: DONE
Implementation note:
- PR #673 is now merged on refreshed `main`; this closeout records durable archive evidence for the bounded TUI command mediation / help-catalog decomposition and promotes the next truthful successor without reopening runtime scope.
Problem:
- After `NA-0225`, refreshed main shows `qsl/qsl-client/qsc/src/tui/controller/commands.rs` as the dominant remaining TUI-local concentration. The next truthful blocker is decomposing command mediation and help/catalog flow into smaller bounded seams without altering the current CLI/TUI, sidecar, marker, or honest-delivery contracts.
Scope:
...
- qsl-attachments remains opaque ciphertext-only
Deliverables:
1) decompose command mediation and help/catalog flow into smaller TUI-local modules
2) keep existing call sites behavior-identical while reducing residual single-file audit radius
3) prove no drift across the representative TUI regressions
4) update governance/evidence truthfully
Acceptance:
1) dominant residual command-mediation concentration is measurably reduced
2) representative suites remain green: `tui_charter.rs`, `tui_product_polish_na0214a.rs`, `tui_fixed_polling.rs`, `tui_relay_drop_reorder.rs`
3) desktop/marker contracts remain green where touched
...
Closeout evidence:
- closeout path: `CY1`
- qsl-protocol implementation PR: #673 https://github.com/QuantumShieldLabs/qsl-protocol/pull/673
- qsl-protocol implementation merge SHA: `e6c4ca216fd4`
- qsl-protocol implementation mergedAt: `2026-04-07T12:54:00Z`
- archive evidence: `docs/archive/testplans/NA-0226_qsc_tui_command_mediation_help_catalog_decomposition_evidence.md`
- exact implementation/evidence outcome:
  - refreshed merged main now carries `DECISIONS.md` `D-0388`, the `TRACEABILITY.md` `NA-0226 implementation/evidence` entry, the merged `qsl/qsl-client/qsc/src/tui/controller/commands.rs` root, the child modules under `qsl/qsl-client/qsc/src/tui/controller/commands/**`, and the merged `qsl/qsl-client/qsc/tests/tui_command_catalog_invariants.rs` surface from PR #673, so the bounded TUI command/help-catalog decomposition is durable on `main` without relying on stale branch or PR state.
  - `qsl/qsl-client/qsc/src/tui/controller/commands.rs` concentration was reduced from `4,024 / 9,456` controller-local lines (`42.55%`) to `2,857 / 9,496` (`30.09%`), while help/catalog ownership plus locked/relay command helpers now live in smaller TUI-local modules under `qsl/qsl-client/qsc/src/tui/controller/commands/**`.
  - representative TUI suites remained green, desktop/marker contracts remained green where touched, and no runtime surfaces outside the approved TUI seam changed in the implementation PR.
  - the implementation landed on PR #673 from refreshed `main`, protected CI completed green before merge, and this closeout PR is governance-only with no runtime-path changes.
  - the next truthful successor is `NA-0227 — qsc TUI State / Poll-Loop Mediation Decomposition` because refreshed residual TUI-local metrics now show `qsl/qsl-client/qsc/src/tui/controller/state.rs` is the dominant remaining concentration inside `qsl/qsl-client/qsc/src/tui/controller/**` at `3,787 / 9,496` lines (`39.88%`), ahead of `commands.rs` at `2,857 / 9,496` (`30.09%`), `render.rs` at `1,044 / 9,496` (`10.99%`), and the retained shell at `451 / 9,496` (`4.75%`).
...
- qsl-attachments remains opaque ciphertext-only
Deliverables:
1) decompose TUI state and poll-loop mediation into smaller TUI-local modules
2) keep existing call sites behavior-identical while reducing residual single-file audit radius
3) prove no drift across the representative TUI regressions
4) update governance/evidence truthfully
Acceptance:
...
Status: DONE
Implementation note:
- PR #677 is now merged on refreshed `main`; this closeout records durable archive evidence for the bounded TUI command residual shell / dispatch decomposition and promotes the next truthful successor without reopening runtime scope.
Problem:
...
Closeout evidence:
- closeout path: `DA1`
- qsl-protocol implementation PR: #677 https://github.com/QuantumShieldLabs/qsl-protocol/pull/677
- qsl-protocol implementation merge SHA: `dca4cb7e127e`
- qsl-protocol implementation mergedAt: `2026-04-09T00:44:41Z`
- archive evidence: `docs/archive/testplans/NA-0228_qsc_tui_command_residual_shell_dispatch_decomposition_evidence.md`
- exact implementation/evidence outcome:
  - refreshed merged main now carries `DECISIONS.md` `D-0392`, the `TRACEABILITY.md` `NA-0228 implementation/evidence` entry, the merged `qsl/qsl-client/qsc/src/tui/controller/commands.rs` root, the child modules under `qsl/qsl-client/qsc/src/tui/controller/commands/**`, and the merged `qsl/qsl-client/qsc/tests/tui_command_catalog_invariants.rs` surface from PR #677, so the bounded TUI command residual shell / dispatch decomposition is durable on `main` without relying on stale branch or PR state.
  - `qsl/qsl-client/qsc/src/tui/controller/commands.rs` concentration was reduced from `2,857 / 9,072` controller-local lines (`31.49%`) to `179 / 9,033` (`1.98%`), while residual command-shell and dispatch flow now live in smaller TUI-local modules under `qsl/qsl-client/qsc/src/tui/controller/commands/**`.
  - representative TUI suites remained green, desktop/marker contracts remained green where touched, and no runtime surfaces outside the approved TUI seam changed in the implementation PR.
  - the implementation landed on PR #677 from refreshed `main`, protected CI completed green before merge, and this closeout PR is governance-only with no runtime-path changes.
  - the next truthful successor is `NA-0229 — qsc TUI State Residual Shell / Ownership Mediation Decomposition` because refreshed residual TUI-local metrics now show `qsl/qsl-client/qsc/src/tui/controller/state.rs` is the dominant remaining concentration inside `qsl/qsl-client/qsc/src/tui/controller/**` at `2,336 / 9,033` lines (`25.86%`), ahead of `commands/contacts.rs` at `1,250 / 9,033` (`13.84%`), `state/ownership.rs` at `1,229 / 9,033` (`13.61%`), `render.rs` at `1,044 / 9,033` (`11.56%`), `commands/locked.rs` at `810 / 9,033` (`8.97%`), and `commands/dispatch.rs` at `762 / 9,033` (`8.44%`).
...
Scope:
- `qsl/qsl-client/qsc/src/tui/controller/state.rs`
- `qsl/qsl-client/qsc/src/tui/controller/state/ownership.rs` only if directly touched by the bounded split
- `qsl/qsl-client/qsc/src/tui/controller.rs` only if directly touched by extraction rewiring
- `qsl/qsl-client/qsc/src/tui/**` only if directly touched by the bounded split
- `qsl/qsl-client/qsc/tests/tui_*.rs`
- `qsl/qsl-client/qsc/tests/output_marker_contract_na0217a.rs` only if directly touched
...
- qsl-attachments remains opaque ciphertext-only
Deliverables:
1) decompose the residual state shell and ownership/account-state flow into smaller TUI-local modules
2) keep existing call sites behavior-identical while reducing residual single-file audit radius
3) prove no drift across the representative TUI regressions
4) update governance/evidence truthfully
Acceptance:
1) dominant residual state/ownership concentration is measurably reduced
2) representative suites remain green: `tui_charter.rs`, `tui_product_polish_na0214a.rs`, `tui_fixed_polling.rs`, `tui_relay_drop_reorder.rs`
3) desktop/marker contracts remain green where touched
4) no protocol/service/wire changes beyond the bounded TUI-local decomposition
...
Closeout evidence:
- qsl-attachments harness PR: #33 https://github.com/QuantumShieldLabs/qsl-attachments/pull/33
- qsl-attachments harness head SHA: `b68a61e7546c`
- qsl-attachments harness merge SHA: `248665c8b85a`
- qsl-protocol evidence PR: #817 https://github.com/QuantumShieldLabs/qsl-protocol/pull/817
- qsl-protocol evidence head SHA: `10f2f8f34e44`
- qsl-protocol evidence merge SHA: `be9b6a88e3f4`
- Decision evidence: D-0534 records the executable qsl-attachments
  retention / cleanup / recovery harness.
- Closeout decision: D-0535 restores NA-0283.
- Chosen retention / cleanup / recovery semantics:
  - request-path expiry cleanup marks expired sessions and objects.
  - expired sessions clear staged parts and resume capability hashes.
  - expired committed objects remove committed ciphertext and fetch
    capability hashes.
  - unexpired committed objects survive cleanup.
  - same-root startup recovery re-exposes only coherent open sessions and
    paired committed objects.
  - incoherent, rejected, partial, expired, or deleted state remains
    unavailable fail-closed under the tested boundaries.
- Outcome:
...
Wire/behavior change allowed? YES only in qsl-attachments repo if test-backed and explicitly scoped.
Crypto/state-machine change allowed? NO.
Docs-only allowed? NO, must include executable qsl-attachments disk/quota/abuse harness or prerequisite stop.
Objective:
- Implement executable qsl-attachments disk pressure / quota / abuse proof,
  including quota rejects, no persistence on rejected writes, cleanup under
  pressure, and no secret/plaintext logging.
Scope:
- qsl-attachments tests/harness under separate explicit qsl-attachments packet
- qsl-attachments minimal implementation only if test-backed and required
- qsl-protocol governance/evidence/testplan
...
- rejected writes do not persist objects.
...
Docs-only allowed? YES.
Closeout evidence:
- qsl-protocol PR `#840` merged NA-0292 as merge `f3048da70143`
  from head `964c35234441`. The lane produced design/testplan evidence
  only: sanitized-error expansion and retention/purge metadata policy are
  scoped for a later executable harness lane and are not implemented here.
- D-0557 records the NA-0292 design. D-0558 records this closeout and
  NA-0293 restoration.
- Post-merge main `public-safety` completed success on `f3048da70143`.
- NA-0292 did not change protocol/crypto implementation, qsp protocol-core,
  cryptographic state machines, runtime behavior, service implementation,
  qsl-server, qsl-attachments, qsc-desktop, website/external repo, README,
  START_HERE, workflows, scripts, Cargo, dependencies, branch protection, or
  public-safety configuration.
- Metadata phase-2 remains incomplete. NA-0292 does not claim anonymity,
  metadata-free messaging, untraceability, external review completion,
  production readiness, public internet readiness, sanitized-error runtime
  implementation, or retention/purge runtime implementation.
Objective:
...
Docs-only allowed? NO, must include executable proof or prerequisite stop.
Objective:
- Implement executable metadata phase-2 harness proof for sanitized-error
  expansion and retention/purge metadata policy designed in NA-0292, without
  claiming anonymity, metadata-free messaging, untraceability, external-review
  completion, or production readiness.
Scope:
- exact executable harness files to be determined by NA-0293 directive
- qsl-protocol governance/evidence/testplan
- no service implementation changes unless separately authorized
- no website changes
Must protect:
...
- executable tests prove any changed behavior.
Deliverables:
1) executable sanitized-error/retention-purge harness or prerequisite stop.
2) evidence/testplan.
3) required CI green.
Acceptance:
1) harness passes or stop is justified.
...
Docs-only allowed? YES.
Objective:
- Closed the operator-action blocker packet after PR #855 proved the required
  website source/deploy/authority bundle remains incomplete. This closeout does
  not implement or authorize website changes.
Completion evidence:
- qsl-protocol PR #855 merged NA-0298 as merge `882b882473c0` from validated
  head `d00358d4a84`.
- PR #855 produced:
  ...
  - `docs/public/WEBSITE_IMPLEMENTATION_HANDOFF.md` operator-action addendum
  - DECISIONS.md D-0572
  - TRACEABILITY.md entry
- Bundle classification remains `OPERATOR_BUNDLE_INCOMPLETE`.
- Website implementation remains blocked until an operator supplies and
  authorizes the exact source repository, branch/base SHA, build, preview,
  deploy target/path, rollback, authority, PR/build permission, edit scope,
  approval rule, and environment status.
- D-0572 records the NA-0298 operator-action blocker packet.
- D-0573 records this closeout and NA-0299 restoration.
- No website or external website repository was mutated.
- No qsl-protocol runtime, protocol-core, crypto, demo, service,
  qsc-desktop, qsl-server, qsl-attachments, qsp, qsc, qsl, qsl-client, apps,
  tools, inputs, formal, workflow, script, Cargo, dependency,
...
- No anonymity, metadata-free, untraceable, external-review-complete,
  ...
  implementation claim is introduced.
...
Closeout evidence:
- qsl-protocol PR #859 merged the NA-0300 core replay/reject/no-mutation
  adversarial harness as merge `bba858f2fa22` from validated head
  `49cd92c1273a`.
- The harness added
  `tools/refimpl/quantumshield_refimpl/tests/na_0300_core_replay_reject_no_mutation.rs`
  plus evidence and testplan files:
...
- The harness emits bounded executable markers for replay reject, malformed
  reject, no mutation on reject, no panic/backtrace, no secret/plaintext leak,
  and the combined NA-0300 proof.
- D-0579 records the executable harness decision.
- D-0580 records this closeout and NA-0301 restoration.
- Post-merge `public-safety`, `qsc-linux-full-suite`,
  `macos-qsc-full-serial`, `qsc-adversarial-smoke`, and
  `qsc-adversarial-miri` completed successfully on merge `bba858f2fa22`.
- Refreshed main already had `rustls-webpki v0.103.13`, so NA-0300A
  dependency remediation packets were skipped and no Cargo files changed in
  NA-0300.
...
5) exact successor recommendation or exact prerequisite stop.
Acceptance:
1) exactly one READY item during execution: NA-0335.
2) NA-0334 is DONE.
...
Closeout evidence:
- qsl-protocol implementation PR: #932
  https://github.com/QuantumShieldLabs/qsl-protocol/pull/932
- qsl-protocol PR #932 head SHA: `cc01d9d6c8b`
- qsl-protocol PR #932 merge SHA: `b98ef3217a35`
- post-merge public-safety completed success on `b98ef3217a35`.
- D-0652 records the NA-0335 qshield demo cover traffic prototype
  implementation/harness proof.
- D-0653 records this closeout and NA-0336 restoration.
- Selected successor:
  `NA-0336 -- Metadata Runtime Padding Bucket Expansion Authorization Plan`
- Exact closeout result:
  - NA-0335 delivered opt-in bounded qshield embedded relay/demo cover
    traffic with synthetic local cover, active-session cover, and batch-fill
    cover only.
  - the prototype inherited NA-0333 cost, quota, retention, purge, backup,
    abuse/DoS, deployment, CI, and operator-stop limits.
  - the executable harness proved policy, caps, retention/purge,
    backup/ops boundary, abuse boundary, secret-free artifacts, real-message
    priority, no recursive cover generation, batching/retry/jitter
    preservation, and service-production boundaries.
  - fixed-rate cover, qsl-server production relay cover, qsl-attachments
    production object cover, and public-internet behavior remain excluded.
  - qshield embedded relay/demo evidence remains distinct from qsl-server and
    qsl-attachments production behavior.
...
Objective:
- Execute the next metadata-runtime timing/traffic-shape lane selected by
  NA-0335: an authorization plan for any future padding bucket expansion, or
  stop on an exact prerequisite.
Must protect:
- no unsupported production, public-internet, external-review, anonymity,
  unsupported metadata-free, or unsupported untraceable claim.
...
- no qsl-server, qsl-attachments, qsc/qsp/protocol/crypto/key-schedule,
  ...
  branch-protection, or public-safety configuration change unless exact future
  scope authorizes it.
- no transport padding expansion implementation unless a later implementation
  lane explicitly authorizes it.
Expected first deliverables:
1) padding bucket expansion authorization plan or exact prerequisite stop.
2) exact review of NA-0319 identifier/default-padding evidence and later
   metadata runtime timing/traffic-shape evidence.
3) explicit cap, compatibility, fallback, and claim-boundary requirements for
   any future padding bucket expansion implementation.
4) explicit qshield demo versus qsl-server/qsl-attachments production
   boundary.
5) exact successor recommendation or exact prerequisite stop.
Acceptance:
1) exactly one READY item: NA-0336.
2) NA-0335 is DONE.
3) D-0652 and D-0653 each exist once.
4) no padding bucket expansion implementation is included unless separately
   authorized.
5) no production-service or cross-repo implementation is included unless
   separately authorized.
6) no unsupported claims in these prohibited families: timing-hidden,
...
Closeout evidence:
- qsl-protocol PR #946 merged the NA-0342 qsl-attachments source/authority
  blocker-resolution evidence as merge `813d4c83d362` from validated head
  `ebb4b8e0aa9c`.
- Post-merge main `public-safety` completed success on `813d4c83d362`.
- D-0666 records `COMPLETE_SOURCE_AUTHORITY`.
- D-0667 records this closeout and restores the selected successor.
- Selected successor: `NA-0343 -- Metadata Runtime qsl-attachments Production
  Size-Class Implementation Authorization Plan`.
...
- qsl-attachments GitHub viewer permission was `ADMIN`; default branch
  protection required strict status check `rust`; latest `rust` workflow run on
...
- Execute the next metadata-runtime qsl-attachments/source/implementation-
  authorization lane selected by NA-0342.
Must protect:
- no unsupported production, public-internet, external-review, anonymity,
  unsupported metadata-free, or unsupported untraceable claim.
- no claim that attachment size, timing metadata, or traffic shape is hidden
...
- no qsl-server, qshield, qsc/qsp/protocol/crypto/key-schedule, dependency,
  ...
  public-safety configuration change unless exact future scope authorizes it.
Expected first deliverables:
1) exact qsl-attachments implementation authorization plan, or exact stop
   evidence if any prerequisite has changed.
2) exact qsl-attachments repo URL, local path, default branch/ref, and base
   SHA, with refreshed source freshness proof.
3) exact allowed and forbidden qsl-attachments files plus qsl-protocol
   governance/evidence files.
4) exact build, test, lint, CI, required-check, rollback, deploy/non-deploy,
...
Closeout evidence:
- qsl-protocol PR #954 merged the NA-0346 qsl-server integration implementation
  authorization plan as merge `39c739eaa17d` from validated head
  `eca5bce9e1ef`; required checks were green on the PR head and post-merge
  `public-safety` completed success on `39c739eaa17d`.
- D-0674 records `IMPLEMENTATION_AUTHORIZATION_READY` for a future
  qsl-server integration implementation harness.
- Selected qsl-server source `/srv/qbuild/work/NA-0237D/qsl-server` at
  `3f28d7433e74` matched live remote `main`, with `ADMIN` viewer permission,
  protected `main`, strict required `rust`, no open qsl-server PRs listed, and
  latest listed qsl-server `main` CI success.
- qsl-server was inspected read-only only; no qsl-server branch, checkout,
  commit, push, PR, build, test, deploy, or runtime mutation occurred.
- qsl-attachments PR #37 evidence remains service-local prerequisite evidence,
  not qsl-server integration proof. qshield embedded relay/demo evidence remains
  reference/oracle evidence only.
- Future qsl-server work must refresh source/authority/CI, open a qsl-server PR
  first, keep deploy/rollback/backup/secrets/public-ingress boundaries explicit,
  and stop on any exact prerequisite regression.
- No claim is introduced that attachment size, timing metadata, traffic shape,
  all metadata, anonymity, metadata-free behavior, untraceability, production
  readiness, public-internet readiness, or external review completion is
  achieved.
- Selected successor: `NA-0347 -- Metadata Runtime qsl-server Integration
  Implementation Harness`.
...
- Execute the next metadata-runtime qsl-server implementation harness selected
  by NA-0346, or stop with exact prerequisite evidence if qsl-server source,
  authority, CI, qsl-attachments contract, deploy/rollback, backup,
  secrets/env, monitoring/logging, public-ingress, or public-claim prerequisites
  regress.
Must protect:
- no unsupported production, public-internet, external-review, anonymity,
  metadata-free, or untraceable claim.
...
- qsl-attachments service-local proof from PR #37 remains prerequisite evidence
  only, not qsl-server integration proof.
- qshield embedded relay/demo proof remains reference/oracle evidence only.
- qsl-server mutation only if the future directive names repository, branch,
  base SHA, allowed files, CI, rollback, deploy, backup, qsl-attachments
  integration, secrets/env, and public-claim boundaries.
- no NA-0347 implementation is authorized by NA-0346 closeout alone.
Expected first deliverables:
1) refresh qsl-server source freshness, mutation authority, CI authority,
   branch protection, open PR state, required checks, and local/remote SHA
   match from live state.
2) create a qsl-server implementation branch from live `main` only if all
   prerequisites remain ready, with qsl-server PR evidence before any
   qsl-protocol companion closeout.
3) implement and prove only the bounded qsl-server integration harness selected
   by NA-0346, preserving route/API, storage/proxy, quota, retention/purge,
   backup, deploy/rollback, secrets/env, monitoring/logging, public-ingress, and
   public-claim boundaries.
...
Closeout evidence:
- qsl-protocol PR #974 merged the NA-0356 key custody/key recovery prerequisite
  plan as merge `66a6f00e8b5d` from validated head `bd901d59c9b2`, and
  post-merge qsl-protocol `public-safety` completed success on
  `66a6f00e8b5d`.
- NA-0356 recorded `KEY_CUSTODY_PARTIAL`, `KEY_RECOVERY_PARTIAL`, and
  `TARGET_TOOL_IMPLEMENTATION_DEFERRED`: future custody direction is an
  operator-held repository secret with offline recovery envelope and no-secret
  evidence handling, and future recovery direction is sealed offline recovery
  plus isolated restore verification before reliance.
- No key, passphrase, private key, repository, off-host destination, backup,
  restore, deploy, rollback, purge, qsl-server implementation,
  ...
  README/START_HERE change, backup-script/timer/fstab mutation, off-host setup,
  secret material handling, or public-claim expansion occurred.
- D-0694 records the prerequisite plan and selected successor.
- D-0695 records this closeout and NA-0357 restoration.
...
- Execute the next metadata-runtime prerequisite lane selected by NA-0356:
  define the restore-drill prerequisites, isolated verification boundary,
  retention/old-key compatibility, monitoring/alerting evidence, backup-plan
  update trigger, and operator-runbook gates that must exist before any
  off-host encrypted backup target/tool implementation or key-handling
  operation can safely proceed.
Must protect:
- no unsupported production/public-internet/external-review/anonymity claims.
- no claim of metadata-free or untraceable behavior.
- no claim that attachment size, timing metadata, or traffic shape is hidden
...
- no off-host setup, backup, restore, deploy, rollback, key generation,
  ...
  implementation change is authorized by this restored READY item.
Expected first deliverables:
1) refresh NA-0356 key custody/key recovery evidence, NA-0355 target/tool
   selection evidence, and local backup/off-host posture read-only.
2) define dry-run and isolated real restore-drill boundaries without generating
   or handling secrets unless a future directive explicitly authorizes it.
3) define old-key/archive retention compatibility, failed-recovery cleanup,
   monitoring/alerting, evidence-artifact, and no-secret fixture expectations.
4) decide whether restore-drill prerequisite planning can proceed under the
   current local backup/local-ops posture or must be blocked by backup-plan or
   workflow-support work.
5) select an exact successor for implementation authorization, blocker
...
4) select the next exact successor without implementing unrelated lanes.
Acceptance:
1) exactly one READY item after closeout: NA-0368.
2) NA-0366 is DONE.
3) D-0714 and D-0715 each exist once.
4) D-0716 records the accepted NA-0367 prerequisite plan.
5) D-0717 records this closeout and NA-0368 restoration.
6) no NA-0368 implementation, real off-host setup, remote connection,
...
Closeout evidence:
- qsl-protocol implementation PR: #996
- qsl-protocol implementation head:
  `071d7b7b8a31ab85f506a258b3ad6b7492cabe08`
- qsl-protocol implementation merge:
  `3b174cd1427239bf76144d675263cd262a921a08`
- post-merge qsl-protocol `public-safety` completed success on
  `3b174cd14272`.
- NA-0367 prerequisite result:
...
- Selected successor:
  `NA-0368 -- Metadata Runtime Off-Host Backup Target Candidate / Host
  Identity Operator Prerequisite Plan`.
- D-0716 records the target-access and host-identity prerequisite plan.
- D-0717 records this closeout and NA-0368 restoration.
- This closeout implements no NA-0368 work and performs no live backup,
  deploy, rollback, real restore, restore target creation/mount/copy,
  off-host setup, remote connection, host-key scan, repository init, tool
  installation, backup-script/timer/fstab mutation, key generation, key
  upload, passphrase collection, private-key inspection, recovery-envelope
  content creation, credential handling, secret handling, qsl-server mutation,
  qsl-attachments mutation, qshield runtime mutation, protocol/crypto/qsc/qsp
  implementation change, dependency change, workflow change, website/public-doc
  change, README change, START_HERE change, or public-claim expansion.
...
- Execute the next metadata-runtime off-host backup target-candidate and host
  identity operator-prerequisite lane selected by NA-0367.
Must protect:
- no unsupported production/public-internet/external-review/anonymity claims.
- no claim of metadata-free or untraceable behavior.
- no claim that attachment size, timing metadata, or traffic shape is hidden
...
- no real off-host connection, host mutation, repository initialization,
  ...
  or secret material handling unless a future exact directive authorizes it.
Expected first deliverables:
1) define target-candidate operator fields, target-class boundary, operator
   target-selection evidence requirements, and host identity evidence path
   without connecting to a target.
2) prove the lane remains no-remote-connection and no-secret unless a future
   directive explicitly authorizes real access.
3) preserve NA-0367 evidence as prerequisite planning only; do not present it
   as target setup, verified host identity, off-host backup completion, real
   restore completion, or complete disaster recovery proof.
4) preserve required future markers including
   ...
   `NA0368_NO_ANONYMITY_CLAIM_OK`, and
   `NA0368_NO_UNTRACEABLE_CLAIM_OK`.
Acceptance:
1) exactly one READY item: NA-0368.
2) NA-0367 is DONE.
3) D-0716 and D-0717 each exist once.
4) D-0718 is absent before NA-0368 work begins.
5) no NA-0368 implementation, real off-host setup, remote connection,
   repository init, tool installation, backup, restore, deploy, rollback,
   backup script/timer/fstab mutation, real restore target creation/mount/copy,
   real key generation, key upload, passphrase collection, private-key
   inspection, recovery-envelope content creation, secret handling, qsl-server
