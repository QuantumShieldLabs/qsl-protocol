...
- NA-0176 READY — mock relay transport Transfer-Encoding normalization + combined-header edge cases (tests-only) — planned focus: deterministic raw-socket regressions in `qsl/qsl-client/qsc/tests/**` for header normalization/list-value chunked rejection and `Transfer-Encoding + Content-Length` combined-header rejection with `pull 204` not-enqueued proofs.
