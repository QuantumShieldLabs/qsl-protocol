...
impl ActorClient {
    pub fn spawn(path: &str) -> Result<Self, String> {
        ...
        let stdin = child
            .stdin
            .take()
            .ok_or_else(|| "actor stdin unavailable".to_string())?;
        let stdout = child
            .stdout
            .take()
            .ok_or_else(|| "actor stdout unavailable".to_string())?;
        Ok(Self {
            child,
            stdin,
    ...
    pub fn call(&mut self, op: &str, params: Value) -> Result<Value, String> {
        ...
        self.stdin
            .flush()
            .map_err(|e| format!("flush actor: {e}"))?;
        ...
        let resp: Value =
            serde_json::from_str(&resp_line).map_err(|e| format!("parse actor response: {e}"))?;
