...
const RETRY_POLICY: &str = "qshield_demo_retry_cadence_v1";
const JITTER_POLICY: &str = "qshield_demo_bounded_jitter_v1";
...
const ROUTE_TOKEN_SENTINEL: &str = "NA0329_ROUTE_TOKEN_SENTINEL_DO_NOT_LEAK";
const RAW_HANDLE_SENTINEL: &str = "NA0329_RAW_HANDLE_SENTINEL_DO_NOT_LEAK";
const CANDIDATE_ACK_SENTINEL: &str = "NA0329_CANDIDATE_ACK_SENTINEL_DO_NOT_LEAK";
const PLAINTEXT_SENTINEL: &str = "NA0329_PLAINTEXT_SENTINEL_DO_NOT_LEAK";
const PADDING_SENTINEL: &str = "NA0329_PADDING_SENTINEL_DO_NOT_LEAK";
const PASSPHRASE_SENTINEL: &str = "NA0329_PASSPHRASE_SENTINEL_DO_NOT_LEAK";
const KEY_SENTINEL: &str = "NA0329_RAW_KEY_SENTINEL_DO_NOT_LEAK";
...
#[test]
fn invalid_retry_jitter_is_bounded_without_delete_state_or_output() {
    ...
    for (idx, expected_retry_delay_ms) in [0_u64, 500, 1000, 2000].iter().enumerate() {
        let output = run_recv_with_jitter_policy(&store, &relay.token);
        ...
        assert_jitter_bounds(entry);
        if idx == 0 {
            assert_eq!(entry.get("last_jitter_ms").and_then(Value::as_u64), Some(0));
            assert!(entry
                .get("last_jitter_class")
                .and_then(Value::as_str)
                .is_none());
        } else {
            assert_eq!(
                entry.get("last_jitter_class").and_then(Value::as_str),
                Some("invalid_candidate_retry")
            );
        }
    }

    let capped = run_recv_with_jitter_policy(&store, &relay.token);
    ...
    let ledger_text = fs::read_to_string(store.join(LEDGER_FILE)).expect("read jitter ledger");
    assert_no_secret_text(&ledger_text, &[&relay.token, &ack]);
    println!("JITTER_ARTIFACT_SECRET_FINDING_COUNT 0");

    marker("NA0329_JITTER_POLICY_OK");
    marker("NA0329_DETERMINISTIC_TEST_JITTER_OK");
    marker("NA0329_INVALID_RETRY_JITTER_BOUNDED_OK");
    marker("NA0329_RETRY_CADENCE_STILL_BOUNDED_OK");
    marker("NA0329_NO_REMOTE_DELETE_BEFORE_VERIFY_OK");
    marker("NA0329_NO_ACCEPTED_STATE_ON_INVALID_JITTER_OK");
    marker("NA0329_NO_OUTPUT_ON_INVALID_JITTER_OK");
    marker("NA0329_NO_SECRET_JITTER_ARTIFACT_OK");
}

#[test]
fn empty_poll_jitter_is_bounded_without_long_sleeps() {
    ...
    for (idx, expected_retry_delay_ms) in [0_u64, 500, 1000, 2000, 2000].iter().enumerate() {
        let output = run_recv_with_jitter_policy(&store, &relay.token);
        assert!(output.status.success());
        let text = combined_output(&output);
    ...
    marker("NA0329_EMPTY_POLL_JITTER_BOUNDED_OK");
}

#[test]
fn valid_ack_delivery_and_ack_retry_remain_fail_closed_under_jitter_lane() {
    ...
    marker("NA0329_VALID_DELIVERY_UNCHANGED_OK");
    ...
    marker("NA0329_ACK_RETRY_JITTER_FAIL_CLOSED_OK");
}

#[test]
fn authorization_and_claim_boundary_markers_are_truthful() {
    marker("NA0329_BOUNDED_JITTER_AUTHORIZATION_OK");
    marker("NA0329_QSHIELD_DEMO_BOUNDARY_OK");
    marker("NA0329_SERVICE_PRODUCTION_BOUNDARY_OK");
    marker("NA0329_NO_TIMING_HIDDEN_CLAIM_OK");
    marker("NA0329_NO_TRAFFIC_SHAPE_HIDDEN_CLAIM_OK");
    marker("NA0329_NO_METADATA_FREE_CLAIM_OK");
    marker("NA0329_METADATA_RUNTIME_BOUNDED_JITTER_OK");
}

#[test]
fn required_markers_are_declared() {
    for marker in MARKERS {
        assert!(marker.starts_with("NA0329_"));
        println!("{marker}");
    }
}
...
fn run_recv_with_jitter_policy(store: &Path, token: &str) -> Output {
...
fn assert_policy(ledger: &Value) {
    assert_eq!(
        ledger.get("policy").and_then(Value::as_str),
        Some(RETRY_POLICY)
    );
    assert_eq!(
    ...
    assert_eq!(
        ledger.get("jitter_policy").and_then(Value::as_str),
        Some(JITTER_POLICY)
    );
    assert_eq!(
        ledger.get("jitter_max_ms").and_then(Value::as_u64),
        Some(JITTER_MAX_MS)
    );
    assert_eq!(
        ledger.get("jitter_composed_cap_ms").and_then(Value::as_u64),
        Some(COMPOSED_CAP_MS)
    );
}

fn assert_jitter_bounds(entry: &Value) {
...
fn unique_temp_dir(name: &str) -> PathBuf {
    std::env::temp_dir().join(format!("qshield-na0329-{name}-{}", unique_suffix()))
}
