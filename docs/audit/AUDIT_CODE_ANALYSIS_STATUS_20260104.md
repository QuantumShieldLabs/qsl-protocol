...
| #4 | HIGH | Weak RNG initialization | tools/refimpl/quantumshield_refimpl/src/crypto/stdcrypto.rs | OPEN (confirmed) | Evidence: stdcrypto.rs:51,99 uses thread_rng; PR #TBD |
| #5 | HIGH | Panic on AEAD operations | tools/refimpl/quantumshield_refimpl/src/crypto/stdcrypto.rs | OPEN (confirmed) | Evidence: stdcrypto.rs:36-38 uses expect(\"encrypt\"); PR #TBD |
| #6 | HIGH | ck_pq_recv not updated on boundary | tools/refimpl/quantumshield_refimpl/src/suite2/ratchet.rs | OPEN (confirmed; needs spec check) | Evidence: suite2/ratchet.rs:365 discards _ck_pq_p; PR #TBD |
| #7 | HIGH | State mutation before send completion | tools/refimpl/quantumshield_refimpl/src/qsp/ratchet.rs | OPEN (confirmed) | Evidence: qsp/ratchet.rs:218-229 updates state pre-send; PR #TBD |
| #8 | HIGH | expect() calls on struct invariants | tools/refimpl/quantumshield_refimpl/src/qsp/types.rs | FIXED (guarded) | Code: types.rs:136-144; Test: handshake_init_encode_fails_closed_on_missing_opk_fields (types.rs); PR #TBD |
| #9 | HIGH | Missing key zeroization | tools/refimpl/quantumshield_refimpl/src/crypto/* | OPEN (confirmed) | Evidence: Cargo.toml:23 includes zeroize; no uses in src; PR #TBD |
| #10 | MEDIUM | Timing side-channel in header decryption | tools/refimpl/quantumshield_refimpl/src/qsp/ratchet.rs; suite2/ratchet.rs | OPEN (confirmed) | Evidence: qsp/ratchet.rs:166-171 early return; PR #TBD |
| #11 | MEDIUM | Nonce reuse on counter overflow | tools/refimpl/quantumshield_refimpl/src/qsp/ratchet.rs | FIXED (verified) | Evidence: qsp/ratchet.rs:194-199 checked_add with ns overflow reject; PR #TBD |
| #12 | MEDIUM | take_mk_skipped leaves stale mk_order | tools/refimpl/quantumshield_refimpl/src/qsp/state.rs | OPEN (confirmed) | Evidence: qsp/state.rs:111-113 removes map only; PR #TBD |
