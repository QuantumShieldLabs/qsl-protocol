...
| Suite-2 DH-ratchet boundary (network observer) | `FLAG_BOUNDARY` presence + a fresh on-wire `DH_pub[32]` on ratchet messages; with ratchet-on-reply the boundary cadence correlates with conversation turn-taking (which message timing/direction already largely expose). These are the first boundary messages on the wire — PQ-reseed boundaries are Stage 2 | avoidable-but-accepted: minor beyond what timing already leaks; the bounded fallback (N=4 / T=15min) prevents long silent gaps | NA-0622 (ENG-0012 Stage 1b-ii, D-1239); qsc `tests/handshake_mvp.rs::dh_ratchet_e2e_*` | ACCEPT + DOCUMENT (operator decision, NA-0622); boundary-cadence obfuscation / cover traffic is tracked as ENG-0022, sequenced after ENG-0012 Stage 2 alongside a holistic metadata pass — no lane may claim metadata-minimized ratcheting until then |
| Suite-2 SCKA advertisement / PQ-reseed boundary (network observer) | `FLAG_PQ_ADV` messages carry an on-wire ML-KEM-768 public key (~1184 B pq-prefix) as a standalone CONTROL envelope; `FLAG_PQ_CTXT` reseed boundaries carry an ML-KEM-768 ciphertext (~1088 B pq-prefix) on an ordinary message. Both ride the standard envelope profile (minimum-size padding applies), but their sizes exceed the small-message pad floor, so PQ control traffic is distinguishable from small text messages; cadence is sparse (advertise on establishment/consume/rotation; reseed first-available then every N_pq=8 sent DH boundaries or T_pq=3600 s). **NA-0625 (ENG-0023) delta:** the advertisement's sealed `body_ct` grows by exactly 32 bytes (the ADVAUTH control-plane MAC now rides as the first 32 bytes of the body plaintext); `pq_prefix`, `pq_bind`, `ad_hdr` and `ad_body` layouts are byte-unchanged. Both control frames stay far above the pad floor, so standard-profile padding absorbs the delta and the observable class is unchanged. NA-0625 also retired the ADV/reseed pack-exclusion rule, so an advertisement may now share a pack with a reseed — a marginal *reduction* in distinguishable send events | avoidable-but-accepted (Operator Decision 4, NA-0624): sparse cadence bounds exposure; a passive observer learns that PQ rekeying occurred and its cadence, not key material | NA-0624 (ENG-0012 Stage 2b, D-1243); NA-0625 (ENG-0023, D-1245); qsc `tests/handshake_mvp.rs::scka_e2e_*` | ACCEPT + DOCUMENT (operator decision, NA-0624; the +32 B ADV body recorded at NA-0625); size/cadence obfuscation and cover traffic stay deferred to ENG-0022 alongside the DH-boundary observable — no lane may claim metadata-minimized ratcheting until then |
