...
Local custody is separate from the Director's verification. Machine A's export was obtained, independently whole-file hash-verified, and banked mode 444 as ACCEPTANCE_EXPORT_machine-A_6bc19fb7bd18.txt. Its footer digest and 115-event contiguous sequence were independently checked. The lock/unlock export (whole-file digest prefix 7885c08ec67a) and Machine B export (4138846e5ceb) were not found locally; their transfer/location is requested. Their complete expected digests are in the acceptance disposition. Neither missing export is claimed locally banked. The Director's explicit acceptance is not replaced by a claim of local acquisition.
...
| Thirty audit filings and harness ratchet SHA | AUDIT-FILING-MAP.md names all thirty ENG-0300–0329 rows and exact source filenames. Protocol report header supplies the ratchet SHA; matched to local ratchet.rs during this continuation. Archive digests in the reports are not Git commit IDs. |
...
The original protocol report header names the ratchet SHA; README.md in the publication draft preserves the complete verification value and reproduction recipe. `AUDIT_harness_exp.rs` and `AUDIT_harness_results.txt` supply the mock experiments for ENG-0300–0302 and the size evidence for ENG-0322. The current ratchet source digest was matched, not its behavior revalidated under StdCrypto.

### Remaining gates

The successor and its placement are approved and banked; READY remains NA-0779 until closeout. The seven publication files are covered by the updated selected scope and placed locally. Final-build observations and Director verification satisfy acceptance and authorize this PR; two exact exports remain owed for local custody. Claims documentation is conditional on a supplied Director draft. SR-16 rows 486–505 were derived at this filing. The final record sequence still requires its existing narrow grants: classification/baseline, final stop naming merge commits and local record tip, installed-hook push, bare remote-tip proof, then freeze. New needle hits invoke C2': classification/supplement and Director direction before retry. No final stop, push, remote-tip change or freeze is claimed.
