...
- **⚠ OPEN verification obligation (NA-0634 caveat, operator-directed 2026-07-11):** NA-0634 left the **responder→initiator** sig-pin OPTIONAL on a REDUNDANCY argument — the responder's primary pin now recomputes the combined `fingerprint(kem_pk, sig_pk)`, which already binds `init.sig_pk`, so the separate signing-key pin was judged redundant. **This redundancy is asserted on REASONING, not proof.** The `QSC.HS.*` formal-model lane (D571 Decision 4) **MUST VERIFY** the responder→initiator authentication — specifically that the combined primary pin makes the separate optional signing-key pin redundant — rather than assume it. Do NOT treat this direction as closed on reasoning alone. Ref: `docs/governance/evidence/NA-0634_as_built.md` §Design decisions (2); D-1258.
