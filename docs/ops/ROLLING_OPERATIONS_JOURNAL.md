...
- Packet A initial pushed commit: `0ecd9c3153e`
- Packet A PR: `#750` / `https://github.com/QuantumShieldLabs/qsl-protocol/pull/750`
...
- The first leak scan exited `1` because it scanned full modified files and therefore reported historical SHAs plus the scanner's own literal scan markers in existing helper code. Classified as a recoverable validation command-shape issue because the repo convention is added-line evidence scanning. Corrective action: rerun against added lines only. Final result: added-line hex32plus pattern count `0`, added-line sensitive-marker count `0`.
...
- Packet A workflow-dispatch bootstrap run `25371617745` failed public-safety because the self-repair bootstrap detector did not recognize the qsc-adversarial cargo-fuzz red-main shape. Classified as recoverable in-scope helper strictness mismatch: the live qsc repair profile already combines the failure log with workflow text for the `Install cargo-fuzz` marker, but the self-repair bootstrap detector used only the raw log. Corrective action: apply the same log-plus-workflow marker proof to the qsc self-repair bootstrap detector. Final local result: qsc self-repair eligibility became true.
- The next local bootstrap proof for PR `#750` exited `1` because the helper PR added the NA-0250B public-safety testplan, while the older self-repair bootstrap path accepted only modified testplan stubs. Classified as recoverable in-scope helper scope mismatch because the directive explicitly authorizes the new Packet A testplan and the path remains a public-safety testplan. Corrective action: allow an added public-safety testplan only when the qsc-adversarial cargo-fuzz self-repair bootstrap profile has already been proven eligible; legacy advisory self-repair still requires modified testplan status. Final local result: `validate-self-repair-bootstrap-pr` admitted PR `#750` with exact head `0ecd9c3153e`.
