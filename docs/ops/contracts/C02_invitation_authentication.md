...
==============================================================================================================
AMENDMENTS AM-3 ONWARD (appended by D-1429; the text above, C02 FINAL and AM-1..AM-2, is NOT rewritten)
==============================================================================================================
Ruled by RULING_NA0783_F01_audit_2026-09-24 (sha256 642a9ff82d42936e16c93136fd230dcc11327a7d3d7fc3c0e65c179f4e09e61d) on
AUDIT_F01_C01_C03_FINDINGS (sha256 512d42751d7a7dc3b26fa68fb100a7bf5c812f646e44fad47adc80bb142001c2), the combined adversarial
audit of C01, C02 and C03 as a set. Each amendment names its finding and the cell it amends; where an amendment and the
text above disagree, the amendment governs. Citations: M = qsl-protocol main d5de33d7; P28 = #1828 e29a07df. The
companion C03 amendments are C03 AMENDMENTS AM-1..AM-16 (docs/ops/contracts/C03_relay_authority_and_recovery.md).
...
74a37b30537cf249076673bee1fec69b026d4b34acd2bdcbc286de24e9f299d0).
| Id | Amends | Amendment |
|---|---|---|
| AM-3 (audit X1 / S3.1 edit (4); FX1, FX2, FX6) | T4 X-d; T3 "For how long"; T5 state failed-expired; T7 (new X6); T2 step 10 (FX6) | T4 X-d: a first-connection record that is TERMINAL -- every candidate it holds is failed-expired, superseded or revoked (AM-8; C03 T4 with C03 AMENDMENTS AM-10) (FX1), and no selection or session was committed -- RELEASES its generation: a later first contact for the same pair mints a new generation, which may replace the terminal record. A LIVE record is never replaced ("a record of one generation never replaces another", P28 handshake/mod.rs:582-585, holds for live records). P28 calls hs_lifecycle_new only when no pending exists (:528-541, else handshake_lifecycle_conflict) and removes no entry (entries are only put, :579-597); the successor also calls it when the existing record is terminal. T3 "For how long": receipts and exact replies are retained until the horizon C03 T6 fixes for EACH side -- the redeemer's (C03 T6, F2) and the inviter's (C03 AMENDMENTS AM-2: recovery_until + 2 x CLOCK_SLACK (FX2), recovery_until read from the slot pull, C03 AMENDMENTS AM-1, and clamped there, FX4); "No timestamp is added to any envelope" stands (recovery_until travels on the relay's pull response, not in an envelope). T5 failed-expired, "Entered when": either side -- the redeemer at C03 T6's horizon; the inviter's candidate at recovery_until + 2 x CLOCK_SLACK with no A2 selected after a final drain of its inbox (C03 AMENDMENTS AM-2) (FX2); a new invite is required and mints a new generation. T7 X6: "a terminal (failed-expired, superseded or revoked) record for I_A/I_B, then I_B redeems a new invite from I_A: a new generation is minted, the terminal record replaced, the session completes"; arm (FX1): "I_B's outgoing attempt ended revoked (the relay answered ERR_INVITE_REVOKED with no delivery, C03 T6 R3a, D5), then I_A issues a new invite and I_B redeems it: R3 passes (AM-4), a new generation is minted, the session completes" (delta LP, LS). T2 step 10 (FX6): an authenticated A1 for a NEW invite from an identity for which this side still holds a LIVE prior-generation responder candidate is refused handshake_lifecycle_occupied (P28 handshake/mod.rs:966-968) with the disposition DEFER: retained un-ACKed, with no write, until that candidate's horizon (C03 AMENDMENTS AM-2), then processed; if the prior candidate was failed-expired there, its record is terminal and released and the retained A1 mints the new generation. It is never disposed as invalid, which would burn the new invite. The spelling and ACK mechanics of DEFER stay C05's (T8 OC6; C03 C3-O6); vector C03 AMENDMENTS AM-5 CL13 |
| AM-4 (audit S3.3; FX1, FX3) | T2 R3; T7 (new C15, C16) | R3 gains a first check for a NEW claimant: a LIVE outgoing attempt or responder candidate for payload.commit already exists (not terminal, AM-3: failed-expired, superseded or revoked) (FX1) -> handshake_lifecycle_conflict (the existing spelling, P28 handshake/mod.rs:540), before any network or durable write; and (FX3) a committed session, or an applied first-connection record, for payload.commit's identity already exists -> contacts_session_exists (the existing spelling, P28 handshake/mod.rs:533; M contacts/mod.rs:935), before any network or durable write, until OC7 decides the known-contact re-handshake. Without them R6 refuses the same cases only AFTER EP6 has claimed the second slot at the relay (C03 T6 R5: the first claim is irrevocable), burning the inviter's second invite and leaving its bundle retained there to recovery_until (the committed-session case: refused by hs_lifecycle_new, P28 :529-534, or coalesced by the outgoing path without an A1, :2434-2438, both after that claim). C15: "a live outgoing attempt to I_A exists; I_B pastes a second code minted by I_A -> handshake_lifecycle_conflict at R3, before network; no operation record, no relay request" (delta DIC caller (redeem)). C16 (FX3): "I_B holds an applied session with I_A; I_B pastes a new code minted by I_A -> contacts_session_exists at R3, before network; no operation record, no relay request" (delta DIC caller (redeem)) |
| AM-5 (audit S2.1) | T5 table "Proves / Does NOT prove" (new row) | Row: evidence "route_cap (T1b field 10, signed)"; proves "the author may DEPOSIT into that mailbox (it holds D)"; does NOT prove "that the author owns or reads it: a peer learns D from any envelope ever sent to it, so a signed route_cap may name a third party's mailbox; a reply pushed there is refused by its owner at step 3 (no such operation) and is lost to its sender until the candidate's horizon (C03 T6; C03 AMENDMENTS AM-2)" |
| AM-6 (audit S5.1) | T5 table "Proves / Does NOT prove" (new row) | Row: "what the relay sees (not evidence)" -- every field of both envelopes, which are signed, not encrypted: both identities' public keys, both commitments, invite_id, attempt_id, relay_ep and both deposit capabilities. It proves nothing to anyone, and none of those fields is confidential from the relay; only the inner handshake secrets and message bodies are protected (C03 AMENDMENTS AM-9) |
| AM-7 (audit S3.4) | T4 X-c | Added: H settles its superseded claim (C03 EP8, idempotent) on the next dispatcher pass, so L's public bundle leaves the relay before recovery_until; H's S is kept until that settle is answered (settle presents S) and is zeroized then, or at the horizon (C03 AMENDMENTS AM-10) |
| AM-8 (audit S7.3) | T5 user-visible states (new row) | Row: state "revoked" (a meaning only; the spelling is C05's) -- entered when the relay answered ERR_INVITE_REVOKED for a known operation with no delivery (C03 T6 R3a, D5); messaging none; a new invite is required |
