...
companion C03 amendments are C03 AMENDMENTS AM-1..AM-15 (docs/ops/contracts/C03_relay_authority_and_recovery.md).
| Id | Amends | Amendment |
|---|---|---|
| AM-3 (audit X1 / S3.1 edit (4)) | T4 X-d; T3 "For how long"; T5 state failed-expired; T7 (new X6) | T4 X-d: a first-connection record that is TERMINAL -- every candidate it holds is failed-expired or superseded, and no selection or session was committed -- RELEASES its generation: a later first contact for the same pair mints a new generation, which may replace the terminal record. A LIVE record is never replaced ("a record of one generation never replaces another", P28 handshake/mod.rs:582-585, holds for live records). P28 calls hs_lifecycle_new only when no pending exists (:528-541, else handshake_lifecycle_conflict) and removes no entry (entries are only put, :579-597); the successor also calls it when the existing record is terminal. T3 "For how long": receipts and exact replies are retained until the horizon C03 T6 fixes for EACH side -- the redeemer's (C03 T6, F2) and the inviter's (C03 AMENDMENTS AM-2: recovery_until + CLOCK_SLACK, recovery_until read from the slot pull, C03 AMENDMENTS AM-1); "No timestamp is added to any envelope" stands (recovery_until travels on the relay's pull response, not in an envelope). T5 failed-expired, "Entered when": either side -- the redeemer at C03 T6's horizon; the inviter's candidate at recovery_until + CLOCK_SLACK with no A2 selected (C03 AMENDMENTS AM-2); a new invite is required and mints a new generation. T7 X6: "a terminal (failed-expired or superseded) record for I_A/I_B, then I_B redeems a new invite from I_A: a new generation is minted, the terminal record replaced, the session completes" (delta LP, LS) |
| AM-4 (audit S3.3) | T2 R3; T7 (new C15) | R3 gains a first check for a NEW claimant: a LIVE outgoing attempt or responder candidate for payload.commit already exists (not terminal, AM-3) -> handshake_lifecycle_conflict (the existing spelling, P28 handshake/mod.rs:540), before any network or durable write. Without it R6 refuses the same case only AFTER EP6 has claimed the second slot at the relay (C03 T6 R5: the first claim is irrevocable), burning the inviter's second invite and leaving its bundle retained there to recovery_until. C15: "a live outgoing attempt to I_A exists; I_B pastes a second code minted by I_A -> handshake_lifecycle_conflict at R3, before network; no operation record, no relay request" (delta DIC caller (redeem)) |
