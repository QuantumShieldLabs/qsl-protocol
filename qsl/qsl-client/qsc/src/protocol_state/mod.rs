...
fn directional_update_inner<T>(peer:&str, expected:Option<u64>, apply:impl FnOnce(&mut crate::directional_delivery::Transaction)->Result<T,&'static str>, receive:bool)->Result<T,DirectionalUpdateError>{
    ...
    let owner=directional_owner_load().map_err(Local)?;
    state.hydrate_owner(peer,&owner).map_err(Local)?;
    ...
    state.encode().map_err(Local)?;
    let key=directional_key(peer).map_err(Local)?;
    let admission=std::cell::Cell::new(false);
    let save=|| ->Result<(), &'static str> {
        // Projection may have materialized owned timeline credit while retaining
        // the event. Reload that owner; never overlay the earlier owner snapshot.
        let current=directional_owner_load()?;
        vault::commit_directional_pair(&key,peer,current.generation,original_generation,&admission,|owner,fresh| {
            let before=fresh.clone();
            *fresh=state.clone();
            fresh.reconcile_owner(peer,&before,owner)
        })
    };
    ...
    let _=receive;
    save().map_err(|error| if receive && admission.get() {
        Apply("TRANSACTION_CAPACITY")
    } else {Local(error)})?;
    #[cfg(feature = "na0780-test-hooks")]
...
pub(crate) fn directional_establish(peer:&str,session:&Suite2SessionState)->Result<(),&'static str>{
    ...
    vault::create_directional_pair(&directional_key(peer)?,peer,state)
}
...
mod directional_receive_error_tests {
    ...
    fn local_origin_and_unknown_errors_propagate() {
        ...
        // typed_body_decode fixes maximum to Some(65536), independent of input.
        // Padding::resolve emits PADDING_LIMIT only for maximum==0 or >65536.
        // Hostile total/payload/profile fields therefore cannot produce that code
        // on receive; they retain their separate LENGTH/SIZE/PROFILE refusals.
        for code in ["vault_write_failed","lock_contended","TRANSACTION_ENCODE","TRANSACTION_TAMPERED","timeline_read_failed","RECEIPT_EPOCH","DECAP","COUNTER_OVERFLOW","future_unknown_error","INTEGRATION_PADDING_LIMIT","INTEGRATION_TRAILING"] {
            assert!(!Apply(code).expected_non_admission());assert!(!Local(code).expected_non_admission());
        }
    }
}

// R02 integrated UNAPPLIED review. Operator allocation: NA-0780/TASK.md,
// R02 approved identifier allocation (2026-09-20). Application/review gates remain.
// Only fresh authenticated successor state may use these namespaces.
pub(crate) struct DirectionalLayout {
    pub(crate) owner_key: &'static str,
    pub(crate) peer_prefix: &'static str,
}
pub(crate) fn approved_directional_layout()->Result<DirectionalLayout, &'static str> {
    Ok(DirectionalLayout {
        owner_key: "na0780_directional_owner_v1",
        peer_prefix: "na0780_directional_transaction_v2/",
    })
}

// Director's conditional aggregate candidate, NOT an operating-limit decision.
// This budgets encoded plaintext VaultPayload, not ciphertext, disk or live heap.
...
pub(crate) struct PeerReserve {
    ...
    // retained bytes already appear in the serialized owner/peer payload.
    pub(crate) control_bound: u64,
    pub(crate) peer_future: u64,
    pub(crate) vault_future: u64,
    pub(crate) control: SessionControlReserve,
}
#[derive(Clone, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
...
pub(crate) struct Charge {
    pub(crate) vault_bytes: u64,
}
// Omit unselected schema/version slots. Application requires an approved layout;
// omission is not permission to load this object under the current profile.
#[derive(Clone, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub(crate) struct SessionControlReserve {
    ...
    pub(crate) control_refs: String,
}
impl CapacityOwner {
    pub(crate) fn remaining_vault_bytes(&self)->Result<usize,&'static str> {
        // projection is a component of Charge.vault_bytes, not an extra charge.
        // All mutable owner-side u64 widths are funded to their maximum. Decimal
        // digits are unchanged by outer JSON escaping. Thus actual scalar bytes
        // plus this credit are constant even when a liability shrinks/grows.
        // The peer transaction and control scalars belong to their outer envelope.
        let width=|g:u64|20-g.to_string().len() as u64;
        let mut total=width(self.generation);
        for p in self.peers.values() {
            total=total.checked_add(width(p.generation)).ok_or("directional_capacity_overflow")?;
            total=total.checked_add(width(p.peer_future)).ok_or("directional_capacity_overflow")?;
            total=total.checked_add(width(p.vault_future)).ok_or("directional_capacity_overflow")?;
            total=total.checked_add(p.vault_future).ok_or("directional_capacity_overflow")?;
        }
        for e in self.entries.values() {
            total=total.checked_add(width(e.generation)).ok_or("directional_capacity_overflow")?;
            if e.projection>e.charge.vault_bytes {return Err("directional_owner_invariant");}
            total=total.checked_add(width(e.projection)).ok_or("directional_capacity_overflow")?;
            total=total.checked_add(width(e.charge.vault_bytes)).ok_or("directional_capacity_overflow")?;
            total=total.checked_add(e.charge.vault_bytes).ok_or("directional_capacity_overflow")?;
        }
        usize::try_from(total).map_err(|_|"directional_capacity_overflow")
    }
    pub(crate) fn check_aggregate(&self, actual_plaintext:usize)->Result<(),&'static str> {
        let promised=self.remaining_vault_bytes()?;
        let total=actual_plaintext.checked_add(promised)
            .and_then(|n|n.checked_add(REVIEW_WRITE_HEADROOM))
            .ok_or("directional_capacity_overflow")?;
        if total>REVIEW_AGGREGATE_CANDIDATE {return Err("directional_aggregate_waiting");}
        Ok(())
    }
    pub(crate) fn peer(&self,peer:&str,sid:&[u8;16])->Result<&PeerReserve,&'static str> {
        let p=self.peers.get(peer).ok_or("directional_reserve_missing")?;
        if p.peer!=peer || &p.sid!=sid {return Err("directional_owner_binding");}
        p.control.validate()?;
        if SessionControlReserve::fixed::<16>(&p.control.sid)?!=p.sid {
...
impl SessionControlReserve {
    fn fixed<const N:usize>(s:&str)->Result<[u8;N],&'static str> {
        use base64::Engine;
        if s.len()!=N.div_ceil(3)*4 {return Err("directional_control_encoding");}
        let bytes=base64::engine::general_purpose::STANDARD.decode(s)
            .map_err(|_|"directional_control_encoding")?;
        if bytes.len()!=N || base64::engine::general_purpose::STANDARD.encode(&bytes)!=s {
            return Err("directional_control_encoding");
        }
        bytes.try_into().map_err(|_|"directional_control_encoding")
    }
    pub(crate) fn validate(&self)->Result<(),&'static str> {
        Self::fixed::<16>(&self.sid)?;
        let request=Self::fixed::<45>(&self.send_request)?;
        if request[0]>2 || (request[0]==0 && request.iter().any(|b|*b!=0)) {
            return Err("directional_control_encoding");
        }
        let epochs=Self::fixed::<154>(&self.recv_epoch_state)?;
        let refs=Self::fixed::<1620>(&self.control_refs)?;
        // Each witness: epoch8,DH32,first-request-slot4,flags1,request-hash32.
        // Occupied bit0, request-seen bit1, advert count bits2..3 (0..2).
        let mut seen=std::collections::BTreeSet::new();
        for e in epochs.chunks_exact(77) {
            let flags=e[44];
            if flags&0xf0!=0 || (flags>>2)>2 {return Err("directional_control_encoding");}
            if flags&1==0 {
                if e.iter().any(|b|*b!=0) {return Err("directional_control_encoding");}
            } else if !seen.insert(&e[..8]) {return Err("directional_control_encoding");}
            if flags&2==0 && (e[40..44].iter().any(|b|*b!=0) || e[45..].iter().any(|b|*b!=0)) {
                return Err("directional_control_encoding");
            }
        }
        let mut slots=std::collections::BTreeSet::new();
        for r in refs.chunks_exact(45) {
            if r[0]==0 {
                if r.iter().any(|b|*b!=0) {return Err("directional_control_encoding");}
            } else if r[0]>4 || !slots.insert(&r[1..13])
                || !epochs.chunks_exact(77).any(|e|e[44]&1!=0 && e[..8]==r[1..9]) {
                return Err("directional_control_encoding");
            }
        }
        Ok(())
    }
    // Use only after core authentication/typed-body validation, on the staged owner.
    // `keep` must compare against staged actual dispositions after receive_promises;
    // deletion of a reference alone never authorizes deletion of a disposition.
    pub(crate) fn prune_refs(&mut self,mut keep:impl FnMut(u64,u32,&[u8;32])->bool)
        ->Result<(),&'static str> {
        use base64::Engine;
        let mut refs=Self::fixed::<1620>(&self.control_refs)?;
        for r in refs.chunks_exact_mut(45) {
            if r[0]!=0 && !keep(u64::from_be_bytes(r[1..9].try_into().unwrap()),
                u32::from_be_bytes(r[9..13].try_into().unwrap()),(&r[13..45]).try_into().unwrap()) {
                r.fill(0);
            }
        }
        self.control_refs=base64::engine::general_purpose::STANDARD.encode(refs);
        Ok(())
    }
    // Call after prune_refs in the same staged transaction. Repeated-numbered
    // request/no-effect advertisement/closure rejection occurs before this call.
    pub(crate) fn retain_control(&mut self,class:u8,epoch:u64,slot:u32,hash:[u8;32],
        effective_confirmed:u32)->Result<(),&'static str> {
        use base64::Engine;
        self.validate()?;
        if !(1..=4).contains(&class) {return Err("directional_control_class");}
        // Class4 is the separately witnessed vacant-target advertisement exception.
        if class!=4 && (slot<effective_confirmed || slot-effective_confirmed>=16) {
            return Err("TRANSACTION_CAPACITY");
        }
        let mut refs=Self::fixed::<1620>(&self.control_refs)?;
        let mut count=0usize;
        for r in refs.chunks_exact(45).filter(|r|r[0]!=0) {
            if u64::from_be_bytes(r[1..9].try_into().unwrap())==epoch {
                count+=1;
                if u32::from_be_bytes(r[9..13].try_into().unwrap())==slot {
                    return if r[0]==class && r[13..45]==hash {Ok(())}
                        else {Err("DISPOSITION_CONFLICT")};
                }
            }
        }
        // Conditional consequence of span16 + two witnessed adverts, not a grant
        // to admit18 arbitrary controls. See the accompanying induction obligations.
        if count>=18 {return Err("TRANSACTION_CAPACITY");}
        let r=refs.chunks_exact_mut(45).find(|r|r[0]==0).ok_or("TRANSACTION_CAPACITY")?;
        r[0]=class;r[1..9].copy_from_slice(&epoch.to_be_bytes());
        r[9..13].copy_from_slice(&slot.to_be_bytes());r[13..45].copy_from_slice(&hash);
        self.control_refs=base64::engine::general_purpose::STANDARD.encode(refs);
        Ok(())
    }
}
...
impl SessionControlReserve {
    // Epoch witnesses outlive pruning of individual receipts. Only authenticated
    // epoch retirement may remove them; no per-message reset or cache-only witness.
    pub(crate) fn admit_epoch_control(&mut self,class:u8,epoch:u64,dh:[u8;32],
        slot:u32,wire_hash:[u8;32],effect:bool)->Result<(),&'static str> {
        use base64::Engine;
        self.validate()?;
        let mut epochs=Self::fixed::<154>(&self.recv_epoch_state)?;
        let existing=epochs.chunks_exact(77).position(|e|e[44]&1!=0
            && u64::from_be_bytes(e[..8].try_into().unwrap())==epoch);
        let i=existing.or_else(||epochs.chunks_exact(77).position(|e|e[44]&1==0))
            .ok_or("RECEIPT_CONTEXT_CAPACITY")?;
        let e=&mut epochs[i*77..(i+1)*77];
        if e[44]&1==0 {e[..8].copy_from_slice(&epoch.to_be_bytes());e[8..40].copy_from_slice(&dh);e[44]=1;}
        if e[8..40]!=dh {return Err("EPOCH_BINDING");}
        match class {
            1 => {
                if e[44]&2!=0 {return Err("TRANSACTION_CAPACITY");}
                e[40..44].copy_from_slice(&slot.to_be_bytes());
                e[45..].copy_from_slice(&wire_hash);e[44]|=2;
            }
            2 => {if slot!=0 || !effect {return Err("ROOT_PARENT_OWNER");}}
            3 => {if !effect {return Err("TRANSACTION_CAPACITY");}}
            4 => {
                if !effect || e[44]>>2>=2 {return Err("TRANSACTION_CAPACITY");}
                e[44]+=4;
            }
            _ => return Err("directional_control_class"),
        }
        self.recv_epoch_state=base64::engine::general_purpose::STANDARD.encode(epochs);
        Ok(())
    }
    pub(crate) fn retire_epoch_witnesses(&mut self,mut live:impl FnMut(u64)->bool)
        ->Result<(),&'static str> {
        use base64::Engine;
        let mut epochs=Self::fixed::<154>(&self.recv_epoch_state)?;
        for e in epochs.chunks_exact_mut(77) {
            if e[44]&1!=0 && !live(u64::from_be_bytes(e[..8].try_into().unwrap())) {e.fill(0);}
        }
        self.recv_epoch_state=base64::engine::general_purpose::STANDARD.encode(epochs);
        Ok(())
    }
    pub(crate) fn control_class(&self,epoch:u64,slot:u32)->Result<Option<u8>,&'static str> {
        let refs=Self::fixed::<1620>(&self.control_refs)?;
        Ok(refs.chunks_exact(45).find(|r|r[0]!=0
            && u64::from_be_bytes(r[1..9].try_into().unwrap())==epoch
            && u32::from_be_bytes(r[9..13].try_into().unwrap())==slot).map(|r|r[0]))
    }
    pub(crate) fn control_refs(&self)->Result<Vec<(u8,u64,u32,[u8;32])>,&'static str> {
        let refs=Self::fixed::<1620>(&self.control_refs)?;
        Ok(refs.chunks_exact(45).filter(|r|r[0]!=0).map(|r| (r[0],
            u64::from_be_bytes(r[1..9].try_into().unwrap()),
            u32::from_be_bytes(r[9..13].try_into().unwrap()),r[13..45].try_into().unwrap())).collect())
    }
}

impl SessionControlReserve {
    pub(crate) fn record_request(&mut self,epoch:u64,slot:u32,hash:[u8;32])->Result<(),&'static str> {
        use base64::Engine;
        let mut r=Self::fixed::<45>(&self.send_request)?;
        if r[0]!=0 && u64::from_be_bytes(r[1..9].try_into().unwrap())==epoch {
            return Err("TRANSACTION_CAPACITY");
        }
        r[0]=1;r[1..9].copy_from_slice(&epoch.to_be_bytes());
        r[9..13].copy_from_slice(&slot.to_be_bytes());r[13..].copy_from_slice(&hash);
        self.send_request=base64::engine::general_purpose::STANDARD.encode(r);Ok(())
    }
    pub(crate) fn confirm_request(&mut self,epoch:u64,slot:u32,hash:[u8;32])->Result<(),&'static str> {
        use base64::Engine;
        let mut r=Self::fixed::<45>(&self.send_request)?;
        if r[0]!=0 && u64::from_be_bytes(r[1..9].try_into().unwrap())==epoch
            && u32::from_be_bytes(r[9..13].try_into().unwrap())==slot && r[13..]==hash {
            r[0]=2;self.send_request=base64::engine::general_purpose::STANDARD.encode(r);
        }
        Ok(())
    }
    pub(crate) fn request_covered(&self,prefixes:&[(u64,u32,u32)])->Result<bool,&'static str> {
        let r=Self::fixed::<45>(&self.send_request)?;
        let epoch=u64::from_be_bytes(r[1..9].try_into().unwrap());
        let slot=u32::from_be_bytes(r[9..13].try_into().unwrap());
        Ok(r[0]==2 && prefixes.iter().any(|(g,confirmed,p)|*g==epoch && slot>=*confirmed && slot<*p))
    }
}
...
impl SessionControlReserve {
    pub(crate) fn fresh(sid:[u8;16])->Self {
        ...
        Self{sid:b.encode(sid),generation:0,grant_peer_epoch:None,
            send_request:b.encode([0;45]),recv_epoch_state:b.encode([0;154]),
            control_refs:b.encode([0;1620])}
    }
}
