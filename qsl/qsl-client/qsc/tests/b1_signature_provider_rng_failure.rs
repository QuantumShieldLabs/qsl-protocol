...
const ROUTE_TOKEN_ALICE: &str = "route_token_alice_na0461_b1_sig";
const ROUTE_TOKEN_BOB: &str = "route_token_bob_na0461_b1_sig__";
const B1_SIGNATURE_LABEL: &str = "QSC.SIG.B1";
...
#[test]
fn b1_signature_rng_failure_writes_no_responder_state_or_b1() {
    let iso = common::TestIsolation::new("na0461_b1_sig_rng_failure");
    let (alice_cfg, bob_cfg) = new_vault_pair(&iso, "b1-sign-failure");
    ...
    assert!(
        ...
        "Bob pending state existed before forced B1 signing failure"
    );
    assert!(
        path_bytes(&session_path(&bob_cfg, "alice")).is_none(),
        "Bob session existed before forced B1 signing failure"
    );
    ...
    assert!(!text.contains("reason=b1_sign"), "{text}");
    ...
    assert_eq!(
        ...
        bob_vault_before,
        "forced B1 signing failure changed Bob vault"
    );
    assert!(
        read_mock_vault_secret(&bob_cfg, "handshake.pending.bob.alice").is_none(),
        "forced B1 signing failure wrote Bob responder pending state"
    );
    assert!(
        path_bytes(&pending_legacy_path(&bob_cfg, "bob", "alice")).is_none(),
        "forced B1 signing failure wrote legacy pending state"
    );
    assert!(
        path_bytes(&session_path(&bob_cfg, "alice")).is_none(),
        "forced B1 signing failure wrote Bob session"
    );
    assert!(
        server.drain_channel(ROUTE_TOKEN_ALICE).is_empty(),
        "forced B1 signing failure emitted B1"
    );

    println!("NA0461_B1_SIGNATURE_PROVIDER_RNG_FAILURE_FORCED_OK");
    println!("NA0461_B1_SIGNATURE_PROVIDER_RNG_FAILURE_SIG_SIGN_FAILED_OK");
    println!("NA0461_B1_SIGNATURE_PROVIDER_RNG_FAILURE_NO_RESPONDER_MUTATION_OK");
    println!("NA0461_B1_SIGNATURE_PROVIDER_RNG_FAILURE_NO_B1_OUTPUT_OK");
}

#[cfg(not(qsc_rng_failure_test_seam))]
#[test]
fn b1_signature_rng_failure_seam_inactive_without_cfg() {
    let iso = common::TestIsolation::new("na0461_b1_sig_normal_build");
    ...
    assert_no_secret_output(&(output_text(&alice_init) + &text));

    println!("NA0461_PRODUCTION_SEMANTICS_UNCHANGED_OK");
}

#[cfg(qsc_rng_failure_test_seam)]
#[test]
fn common_na0461_markers() {
    println!("NA0461_B1_SIGNATURE_PROVIDER_RNG_SEAM_IMPLEMENTED_OK");
    println!("NA0461_A2_SIGNATURE_RESIDUAL_DEFERRED_OK");
    println!("NA0461_IDENTITY_PROVIDER_RNG_RESIDUAL_DEFERRED_OK");
    println!("NA0461_X25519_RESIDUAL_DEFERRED_OK");
    println!("NA0461_REFIMPL_PROVIDER_RNG_RESIDUAL_DEFERRED_OK");
    println!("NA0461_KEM_PROVIDER_RNG_BACKGROUND_PRESERVED_OK");
    println!("NA0461_NO_DEPENDENCY_CHANGE_OK");
    println!("NA0461_NO_WORKFLOW_CHANGE_OK");
    println!("NA0461_NO_PUBLIC_READINESS_CLAIM_OK");
    println!("NA0461_NO_CRYPTO_COMPLETE_CLAIM_OK");
    println!("NA0461_NO_SIGNATURE_COMPLETE_CLAIM_OK");
    println!("NA0461_NO_IDENTITY_COMPLETE_CLAIM_OK");
    println!("NA0461_NO_RNG_FAILURE_COMPLETE_CLAIM_OK");
    println!("NA0461_NO_PROVIDER_RNG_COMPLETE_CLAIM_OK");
    println!("NA0461_ONE_READY_INVARIANT_OK");
}
