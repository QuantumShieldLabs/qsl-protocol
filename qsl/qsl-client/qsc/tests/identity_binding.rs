...
fn pinned_mismatch_rejected_no_mutation() {
    ...
    contacts_add_authenticated_with_route(&alice2_cfg, "bob", bob_fp.as_str(), bob_kem.as_str(), bob_sig.as_str(), ROUTE_TOKEN_BOB);
...
fn handshake_accepts_verification_code_pin_without_peer_mismatch() {
    ...
    let bob_sig = bob_show_text
        .lines()
        .find_map(|line| line.strip_prefix("identity_sig_pk="))
        .expect("bob identity sig line");
