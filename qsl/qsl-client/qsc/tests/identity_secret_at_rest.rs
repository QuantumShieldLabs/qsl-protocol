...
fn identity_kem_pk(cfg: &Path, label: &str) -> String {
    ...
    output_str(&out)
        ...
        .map(ToOwned::to_owned)
        .unwrap_or_else(|| panic!("missing identity_kem_pk marker: {}", output_str(&out)))
}
...
fn identity_sig_pk(cfg: &Path, label: &str) -> String {
    ...
    output_str(&out)
        ...
        .map(ToOwned::to_owned)
        .unwrap_or_else(|| panic!("missing identity_sig_pk marker: {}", output_str(&out)))
}
...
fn migrate_legacy_identity_into_vault() {
    ...
    contacts_add_authenticated_with_route(&cfg, "bob", bob_fp.as_str(), bob_kem.as_str(), bob_sig.as_str(), ROUTE_TOKEN_BOB);
