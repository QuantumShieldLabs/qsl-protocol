...
fn identity_sig_pk(cfg: &Path, label: &str, passphrase: &str) -> String {
    let out = qsc_with_unlock(cfg, passphrase)
        .args(["identity", "show", "--as", label])
        .output()
        .expect("identity show");
    ...
    combined_output(&out)
        ...
        .map(ToOwned::to_owned)
        .unwrap_or_else(|| panic!("missing identity_sig_pk marker: {}", combined_output(&out)))
}
...
fn receive_mailbox_peer_separation_fail_closed() {
    ...
    let alice_sig = identity_sig_pk(&alice_cfg, "alice", "test-pass-a");
    let bob_fp = identity_fp(&bob_cfg, "bob", "test-pass-b");
    let bob_kem = identity_kem_pk(&bob_cfg, "bob", "test-pass-b");
    let bob_sig = identity_sig_pk(&bob_cfg, "bob", "test-pass-b");
