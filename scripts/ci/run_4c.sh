...
ART_DIR="$ROOT/artifacts/$RUN_ID/4C"
EVI_DIR="$ROOT/evidence/4C_runs/$RUN_ID/4C"
...
install_refimpl_actor() {
  ...
  if [[ ! -f "$src" ]]; then
    echo "[4C] ERROR: expected actor binary not found at $src" >&2
    return 2
  fi
...
build_refimpl_actor() {
  if ! command -v cargo >/dev/null 2>&1; then
    echo "[4C] ERROR: cargo not found; Rust toolchain is required for refimpl actors." >&2
    return 2
  fi

  echo "[4C] building Rust refimpl_actor (release)" >&2
...
start_inprocess_stubs() {
  echo "[4C] docker compose not available; starting in-process stubs on 18080/18081/18082" >&2
...
echo "[4C] Run ID: $RUN_ID"
echo "[4C] Artifacts: $ART_DIR"
echo "[4C] Evidence:   $EVI_DIR"
echo "[4C] Actors:     $ACTORS_CONFIG"
...
python3 "$ROOT/tests/harness/4b/runner.py" interop \
  ...
  --profile extended \
  --out-name "C1_interop_extended.json" 2>&1 | tee "$ART_DIR/C1_interop_extended.log"
rc_C1=${PIPESTATUS[0]}

set -e

if [[ "${rc_B0:-0}" -ne 0 || "${rc_C1:-0}" -ne 0 ]]; then
  echo "[4C] One or more stages failed (preflight=$rc_B0 interop_extended=$rc_C1)" >&2
  exit 1
fi
