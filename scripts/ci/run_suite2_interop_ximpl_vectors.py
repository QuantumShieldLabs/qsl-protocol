...
def main() -> int:
    ...
    ap.add_argument("--actor-a", required=True, help="Path to actor A (refimpl_actor)")
    ap.add_argument("--actor-b", required=True, help="Path to actor B (interop python actor)")
    ap.add_argument("--actor-name", default="suite2-ximpl")
    ap.add_argument("--file", default="inputs/suite2/vectors/qshield_suite2_interop_ximpl_vectors_v1.json")
    ap.add_argument("--out", default="artifacts/suite2/interop_ximpl_vector_report.json")
    args = ap.parse_args()

    vs = jload(Path(args.file))
    vectors = [v for v in vs.get("vectors", []) if "CAT-S2-INTEROP-XIMPL-001" in (v.get("tags") or [])]
    if not vectors:
        print("No CAT-S2-INTEROP-XIMPL-001 vectors found (fail-closed).", file=sys.stderr)
        return 2

    report: Dict[str, Any] = {
        "ok": True,
        "category": "CAT-S2-INTEROP-XIMPL-001",
    ...
    for v in vectors:
        ...
        if isinstance(sender, dict) and sender.get("type") == "json":
            sender = sender.get("data")
        if isinstance(sender, dict):
            sender = sender.get("role") or sender.get("value")
        flags = inp.get("flags")

        if plaintext is None or sender not in ("A", "B"):
            ok_all = False
            report["failed"] += 1
            report["failures"].append({"id": vid, "op": op, "error": "missing plaintext_hex/sender"})
            continue

        actor_a = ActorProc.start(args.actor_name + "-A", args.actor_a)
        actor_b = ActorProc.start(args.actor_name + "-B", args.actor_b)
        ...
        try:
            if use_establish:
                ...
                ra = actor_a.request("suite2.establish.run", establish_a, rid=f"{vid}:estA")
                if not ra.get("ok"):
                    ok_all = False
                    report["failed"] += 1
                    report["failures"].append({"id": vid, "op": op, "error": f"establish A failed: {ra}"})
                    continue
                rb = actor_b.request("suite2.establish.run", establish_b, rid=f"{vid}:estB")
                if not rb.get("ok"):
                    ok_all = False
                    report["failed"] += 1
            ...
            actor_send = actor_a if sender == "A" else actor_b
            actor_recv = actor_b if sender == "A" else actor_a
            ...
            send_resp = actor_send.request("suite2.e2e.send", send_params, rid=f"{vid}:send")
            ...
            recv_resp = actor_recv.request("suite2.e2e.recv", recv_params, rid=f"{vid}:recv")
    ...
    if not report["ok"]:
        print(f"Suite-2 interop ximpl vectors FAILED: {report['failed']} failing of {report['ran']}", file=sys.stderr)
        for f in report["failures"][:10]:
            print(f"- {f.get('id')} {f.get('op')}: {f.get('error','mismatch')}", file=sys.stderr)
        return 2

    print(f"Suite-2 interop ximpl vectors OK: {report['passed']} / {report['ran']}")
    return 0
