...
def main() -> int:
    ...
    ap.add_argument("--actor-name", default="suite2-ooo")
    ap.add_argument("--file", default="inputs/suite2/vectors/qshield_suite2_ooo_replay_vectors_v1.json")
    ap.add_argument("--out", default="artifacts/suite2/ooo_replay_vector_report.json")
    args = ap.parse_args()

    vs = jload(Path(args.file))
    vectors = [v for v in vs.get("vectors", []) if "CAT-S2-OOO-001" in (v.get("tags") or [])]
    if not vectors:
        print("No CAT-S2-OOO-001 vectors found (fail-closed).", file=sys.stderr)
        return 2

    report: Dict[str, Any] = {
        "ok": True,
        "category": "CAT-S2-OOO-001",
    ...
    if not report["ok"]:
        print(f"Suite-2 ooo/replay vectors FAILED: {report['failed']} failing of {report['ran']}", file=sys.stderr)
        for f in report["failures"][:10]:
            print(f"- {f.get('id')} {f.get('op')}: {f.get('error','mismatch')}", file=sys.stderr)
        return 2

    print(f"Suite-2 ooo/replay vectors OK: {report['passed']} / {report['ran']}")
    return 0
