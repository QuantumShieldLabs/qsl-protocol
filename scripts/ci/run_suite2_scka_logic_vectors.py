...
def main() -> int:
    ...
    ap.add_argument("--file", default="inputs/suite2/vectors/qshield_suite2_scka_logic_vectors_v1.json")
    ap.add_argument("--out", default="artifacts/suite2/scka_logic_vector_report.json")
    args = ap.parse_args()

    vs = jload(Path(args.file))
    vectors = [v for v in vs.get("vectors", []) if "CAT-SCKA-LOGIC-001" in (v.get("tags") or [])]
    if not vectors:
        print("No CAT-SCKA-LOGIC-001 vectors found (fail-closed).", file=sys.stderr)
        return 2

    report: Dict[str, Any] = {
        "ok": True,
        "category": "CAT-SCKA-LOGIC-001",
        ...
        if exp_ok is True:
            ...
            if reason:
                ...
                if reason not in msg:
                    ...
                    continue
            if exp.get("state_unchanged") is True:
                # Compare echoed state (if provided) to input state (fail-closed if missing)
                got_state = (resp.get("result") or {}).get("state")
                if got_state is None:
                    ok_all = False
                    report["failed"] += 1
                    report["failures"].append({"id": vid, "op": op, "error": "state_unchanged asserted but actor did not return result.state"})
                    continue
                in_state = inp.get("state")
                if not deep_equal(got_state, in_state):
                    ok_all = False
                    report["failed"] += 1
                    report["failures"].append({"id": vid, "op": op, "error": "state changed unexpectedly", "mismatch": {"expect_state": in_state, "got_state": got_state}})
                    continue
            report["passed"] += 1
    ...
    if not report["ok"]:
        print(f"SCKA logic vectors FAILED: {report['failed']} failing of {report['ran']}", file=sys.stderr)
        for f in report["failures"][:10]:
            print(f"- {f.get('id')} {f.get('op')}: {f.get('error','mismatch')}", file=sys.stderr)
        return 2

    print(f"SCKA logic vectors OK: {report['passed']} / {report['ran']}")
    return 0
