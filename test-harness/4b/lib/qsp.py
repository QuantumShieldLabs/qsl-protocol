...
def qsp_parse(b: bytes) -> dict:
    ...
    need(2); protocol_version = struct.unpack_from(">H", b, off)[0]; off += 2
    need(2); suite_id = struct.unpack_from(">H", b, off)[0]; off += 2
    ...
    need(16); session_id = b[off:off+16]; off += 16
    need(32); dh_pub = b[off:off+32]; off += 32
    need(2); flags = struct.unpack_from(">H", b, off)[0]; off += 2
    need(12); nonce_hdr = b[off:off+12]; off += 12
    ...
    if flags & FLAG_PQ_ADV:
        need(4); pq_adv_id = struct.unpack_from(">I", b, off)[0]; off += 4
        need(1184); pq_adv_pub = b[off:off+1184]; off += 1184
    else:
        pq_adv_id = None
        pq_adv_pub = None

    if flags & FLAG_PQ_CTXT:
        need(4); pq_target_id = struct.unpack_from(">I", b, off)[0]; off += 4
        need(1088); pq_ct = b[off:off+1088]; off += 1088
    else:
        pq_target_id = None
        pq_ct = None

    need(2); hdr_ct_len = struct.unpack_from(">H", b, off)[0]; off += 2
    if hdr_ct_len != 24:
        raise QSPError("hdr_ct_len must be 24")
    need(hdr_ct_len); hdr_ct = b[off:off+hdr_ct_len]; off += hdr_ct_len

    need(4); body_ct_len = struct.unpack_from(">I", b, off)[0]; off += 4
    if body_ct_len < 16:
        raise QSPError("body_ct_len too small")
    need(body_ct_len); body_ct = b[off:off+body_ct_len]; off += body_ct_len
