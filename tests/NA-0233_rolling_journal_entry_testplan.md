...
- Confirm `DECISIONS.md` appends `D-0406` and records the resolved MockProvider fixed-key runtime truth as implementation/evidence only on top of the refreshed `D-0403` through `D-0405` CI-lane history now on `main`.
...
- Confirm the same rolling journal file carries the `DIRECTIVE 288` refreshed-main resume entry for PR `#688`, including the in-scope merge-resolution proof and the `resume_in_place` decision.
