...
# NA-0363 Closeout Restore NA-0364 Testplan

## Objective

Verify that NA-0363 closes only after PR #988 post-merge public-safety is green, and that the queue restores exactly one successor: NA-0364 -- Metadata Runtime Restore Drill Isolated Restore Authorization Plan.

## Protected invariants

- NA-0363 is DONE.
- NA-0364 is the only READY item.
- D-0708 exists once.
- D-0709 exists once.
- D-0710 is absent.
- no NA-0364 implementation.
...
- no production-readiness/public-internet-readiness/external-review-complete/anonymity/metadata-free/untraceable/hidden-size/hidden-timing/hidden-traffic-shape/off-host-backup-complete/disaster-recovery-complete claim.
...
- `tests/NA-0363_closeout_restore_na0364_testplan.md`

## Forbidden scope

- NA-0364 implementation.
- qsl-server mutation.
...
- `python3 scripts/ci/qsl_evidence_helper.py queue` reports `READY NA-0364`.
- `NEXT_ACTIONS.md` marks NA-0363 DONE.
- `NEXT_ACTIONS.md` does not mark NA-0364 DONE or IN PROGRESS.

## Decision requirements

- `python3 scripts/ci/qsl_evidence_helper.py decisions` reports duplicate decision count zero.
- D-0708 appears exactly once.
- D-0709 appears exactly once.
- D-0710 is absent.
- D-0709 states that NA-0363 delivered no-secret target/tool harness evidence and that NA-0364 is only restored, not implemented.
...
- Added text may describe NA-0363 as no-secret simulated target/tool harness evidence only.
- Added text must not describe NA-0363 as real off-host backup completion.
- Added text must not describe NA-0364 as implemented.
...
- `python3 scripts/ci/qsl_evidence_helper.py scope-guard --base origin/main --allowed NEXT_ACTIONS.md --allowed DECISIONS.md --allowed TRACEABILITY.md --allowed docs/ops/ROLLING_OPERATIONS_JOURNAL.md --allowed tests/NA-0363_closeout_restore_na0364_testplan.md`
- `python3 scripts/ci/qsl_evidence_helper.py link-check`
- `python3 scripts/ci/qsl_evidence_helper.py leak-scan --mode added --base origin/main`
- `bash scripts/ci/classify_ci_scope.sh NEXT_ACTIONS.md DECISIONS.md TRACEABILITY.md docs/ops/ROLLING_OPERATIONS_JOURNAL.md tests/NA-0363_closeout_restore_na0364_testplan.md`
...
- NA-0364 is the exact sole READY successor.
- NA-0364 objective is authorization planning for isolated restore-drill work after no-secret restore dry-run, key custody/recovery, and off-host target/tool harness evidence.
- NA-0364 must continue to protect against real restore execution, real restore target creation/mount/copy, real key handling, off-host backup setup, backup/restore/deploy/rollback operation, qsl-server/qsl-attachments production-boundary drift, and unsupported public/privacy/readiness claims unless a future exact directive authorizes otherwise.
