...
- **Live GUI acceptance (§B): FLOWN AND PASSED** (operator-flown, 2026-07-24). All
  ...
  LAN relay rig over real TLS — no mocks. Per-check results + screenshot references
...
**Rig:** the LAN relay host `https://<lan-address>:8443` — qsl-server (current main, the C-2
...
|---|---|---|---|---|
| 1 | Reachable{Bearer} | **bearer** | CA set (`ca-set`), valid token set, URL = the LAN relay host | banner **Connected** (neutral); "Token required — accepted. Certificate trusted."; doc rows (relay name, Certificate=Trusted, Access, retention, max size, version) show the REAL doc values |
...
| 5 | Certificate not trusted | either | CA CLEARED (bare self-signed), URL = the LAN relay host | "Certificate not trusted" (accent); the interception-attack copy; remedy = add the CA |
...
| 8 | Save-state | bearer | after check 1, EDIT the URL to a new valid LAN-relay-host form and Test again | "Not saved yet." + Save takes the accent (primary) treatment; press **Save** → note clears, Save returns to secondary; then EDIT any field → the results panel CLEARS (state 10) |
