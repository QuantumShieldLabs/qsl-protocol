...
def qse_parse(b: bytes, policy: Policy) -> QSEEnvelope:
    ...
    need(2)
    env_version = struct.unpack_from(">H", b, off)[0]
    off += 2
    need(2)
    ...
    need(2)
    rt_len = struct.unpack_from(">H", b, off)[0]
    off += 2
    if rt_len > 512:
        raise BoundsExceeded("route_token too long")
    need(rt_len)
    route_token = b[off:off + rt_len]
    off += rt_len

    need(4)
    timestamp_bucket = struct.unpack_from(">I", b, off)[0]
    off += 4
    need(2)
    pad_len = struct.unpack_from(">H", b, off)[0]
    off += 2
    need(4)
    payload_len = struct.unpack_from(">I", b, off)[0]
    off += 4
    ...
    need(payload_len)
    payload = b[off:off + payload_len]
    off += payload_len
    need(pad_len)
    pad = b[off:off + pad_len]
    off += pad_len
