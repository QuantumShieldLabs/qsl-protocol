...
impl SessionState {
    ...
    pub fn store_hk_skipped(&mut self, old_dh_peer: [u8; 32], hk_r: [u8; 32], nhk_r: [u8; 32]) {
        if self.hk_skipped.contains_key(&old_dh_peer) {
            return;
        }
    ...
    pub fn mk_skipped_len(&self) -> usize {
        self.mk_skipped.len()
    }
    ...
    pub fn snapshot_bytes(&self) -> Vec<u8> {
        ...
        let mut mk_entries: Vec<([u8; 32], u32, [u8; 32])> = self
            .mk_skipped
            .iter()
            .map(|(k, v)| (k.0, k.1, *v))
            .collect();
        ...
        let mut pq_entries: Vec<(u32, Vec<u8>, Vec<u8>)> = self
            .pq_self
            .iter()
        ...
        impl<'a> Cur<'a> {
            fn take(&mut self, n: usize) -> Result<&'a [u8], StateError> {
                if self.i + n > self.b.len() {
                    return Err(StateError::Invalid("bad snapshot"));
                }
            ...
            fn opt_arr32(&mut self) -> Result<Option<[u8; 32]>, StateError> {
                ...
                if f == 0 {
                    ...
                    Err(StateError::Invalid("bad snapshot"))
                }
            }
...
pub fn derive_header_keys_kmac(
    ...
    match role {
        SessionRole::Initiator => (
            k(kmac, rk, "QSP4.3/HK/A->B"),
            k(kmac, rk, "QSP4.3/HK/B->A"),
            k(kmac, rk, "QSP4.3/NHK/A->B"),
            k(kmac, rk, "QSP4.3/NHK/B->A"),
        ),
...
mod tests {
    ...
    fn store_mk_skipped_rejects_deterministically_and_no_state_mutation_on_failure() {
        ...
        let err1 = st1
            .store_mk_skipped([0xFFu8; 32], 99, [0x11u8; 32])
            .unwrap_err();
